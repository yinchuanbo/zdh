    ...
    if (problems > 0) {
        const errorMessages = results.flatMap(result =>
            result.messages.map(msg => {
                return `<div class="js-error">Error: <span>${msg.ruleId || 'unknown rule'}</span> <span>at ${result.filePath} (line ${msg.line}, column ${msg.column})</span>: <span>${msg.message}</span></div>`;
            })
        );
